...
class _EditSellerProfileState extends State<EditSellerProfile> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Edit Seller Profile', style: TextStyle(color: Color(0xFFFF5C01), fontSize: 28, fontWeight: FontWeight.bold)),
      ),
                ...
                CustomButton(
                  onPressed: () async {
                    if (formKey.currentState!.validate()) {
                      ...
                      try {
                        final response = await ApiServices.updateSeller(widget.userId, data);

                        if (response['status'] != 'error') {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) =>  ProfilePage(), //SellerProfile(userId: widget.userId),
                            ),
                          );
                        } else {
